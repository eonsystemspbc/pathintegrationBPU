...
CELLS = {
    ...
    ("optic lobe", 3):    (0.0,  "0.993 vs 0.993", False),  # foreign: image classification
    ...
    ("mushroom body", 3): (-0.1, "0.993 vs 0.993", False),
    ...
    ("central complex", 3): (0.0, "0.993 vs 0.993", False),
...
ax.set_title("Connectome vs random-control advantage across the region × task matrix\n"
             "positive = connectome beats its random null  •  black box = native/matched task  •  ✗ = foreign, null",
             fontsize=12.5, pad=22)
cbar = fig.colorbar(im, ax=ax, shrink=0.8, pad=0.02)
cbar.set_label("connectome advantage over random (%)", fontsize=10)
fig.text(0.5, 0.005,
         "Flow = REAL DSEC event-camera flow. Native diagonal carries the advantage (flow/OL, path/CX); MQAR is generic (OL≈MB, not MB-specific). "
         "Foreign tasks (image classification, arithmetic) are null for every region → the wiring is NOT a general substrate. "
         "'sort' excluded (recurrence-using, small generic residual). Flow/path 1 seed.",
