...
- [`bpu_reproduction_gap_analysis.md`](bpu_reproduction_gap_analysis.md) — which BPU-paper claims did /
  didn't reproduce.
- [`pool_fidelity/`](pool_fidelity/README.md) — how biological are the sensory/output **ports**?
  Measures the extra-regional-flow pools against known cell types (CX output ~94% out-projecting;
  hemibrain MB internal≈KC, output≈MBON). The connectome-vs-control comparison is invariant to this
  (controls share the same pools); generated by `scripts/connectome/report_pool_fidelity.py`.
