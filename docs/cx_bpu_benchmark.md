...
```bash
python experiments/hemibrain_cx_bpu/scripts/patent/plan_patent_experiments.py \
...
in `scripts/associative/run_mb_associative_learning.py`. It pairs sparse odor signatures
...
`scripts/associative/run_omniglot_associative_benchmark.py`. It evaluates the
...
`scripts/associative/run_meta_album_associative_benchmark.py`. It applies the same
...
`scripts/associative/run_ccnlab_associative_benchmark.py`. It runs the same
...
`scripts/flow/run_dsec_flow_benchmark.py`. It trains a DSEC-Flow model with a
...
`scripts/associative/run_multi_gpu_associative_sweep.py` to run independent model/seed jobs
...
can be summarized with `scripts/associative/summarize_associative_sweep.py`, which writes
...
`scripts/figures/plot_ccnlab_learning_curve.py` to inspect acquisition/extinction
...
`scripts/transfer/run_cross_region_transfer.py`. It can train the CX substrate on the
...
```bash
WHOLE_OUT=/home/ubuntu/pathintegrationBPU/connectomes/flywire_whole_bump_seed0
mkdir -p "$WHOLE_OUT"
...
```bash
python scripts/flow/run_optic_flow_benchmark.py --help
```
...
```bash
python scripts/flow/run_tartanair_optic_flow_benchmark.py --help
```
