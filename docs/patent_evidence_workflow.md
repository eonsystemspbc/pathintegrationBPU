...
- `scripts/patent/plan_patent_experiments.py` emits AWS-ready commands for the selector, matched/mismatched region experiments, optic flow, low-power proxy, and final report.
- `scripts/benchmarks/run_low_power_proxy_benchmark.py` estimates sparse-vs-dense recurrent memory and operation footprint.
- `scripts/patent/make_patent_evidence_report.py` aggregates selectors, metrics, manifests, and missing-evidence checks into one Markdown report.
...
```bash
python scripts/patent/plan_patent_experiments.py --plan-dir outputs/patent_evidence_plan --output-root outputs --seeds 0 1 2 3 4 --epochs 40 --device cuda
```
