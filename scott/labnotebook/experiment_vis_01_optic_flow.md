...
**Status (updated 2026-07-12):** **Subruns 03 + 04 finished — the connectome FlowRNN floors on EVERY substrate.**
...
**Next step is a model/training fix, not more seeds or another substrate.** Candidate directions: break the
fixed-point collapse (state initialization / leak, spectral placement of the trainable recurrence, or an
explicit temporal-derivative loss term), and validate any fix on `mb_core_alpn` (cheapest) before rerunning
the optic lobe. Only once a substrate clears the fair GRU bar does the original subrun-02
connectome-vs-degree-matched comparison become runnable.
