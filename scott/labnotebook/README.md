...
| vis-01 | 2026-07-09 | Optic-lobe connectome vs degree-matched controls on optic flow (reading self-motion from a fly-eye movie) | **Blocked on a model/training fix — subruns 03 + 04 finished and the connectome FlowRNN floored on EVERY substrate (updated 2026-07-12).** The yaw-only learnability run completed on the fleet: 20 optic-lobe + 40 mushroom-body seeds (`mb_full` + `mb_core_alpn`) all trained to 300 epochs, and **none of the 60 networks cleared held-out R² ≈ 0** vs a GRU ceiling of 0.58 (causal) / 0.76 (bidirectional) on the identical stimulus. Optic lobe and mushroom body floor **equally** → the pre-registered *model/training* outcome (these sparse connectome FlowRNNs are hard to train on continuous regression), **not** a vision-specific one; the headline connectome-vs-control test (subrun 02) stays blocked until a fix gets a substrate above floor (validate on cheap `mb_core_alpn` ~3 h/run before the ~26 h/run optic lobe). First experiment of the optic-lobe (`vis`) track and its go/no-go gate — the vision analogue of mb-01: does the FlyWire **left optic-lobe** connectome's wiring beat random rewiring at reading the fly's own motion from what its eye sees (generic wiring-only I/O)? Real substrate built (**48,894 neurons / 4,205,392 signed edges**, ρ→0.95); fresh fly-hex-eye + 3D cluttered-scene movie generator; `FlowRNN` (sparse-trainable recurrence). **Subrun 02** (the definitive 5-DOF continuous-rotation run, connectome ×20 vs degree-matched ×20) ran on the AWS fleet and **both arms floored at val R² ≈ 0** — a training/model failure, not a connectome-vs-control signal. Debugging on a reduced **yaw-only** stimulus showed the *signal is learnable* (a GRU reference hits **R² ≈ 0.74**) while the sparse FlowRNN **memorizes a batch (0.92) but doesn't generalize**, and the shipped **activity-normalization default diverged** (real bug). Fixes: **detach the RMS-norm denominator** (`model.py`) and add a first-class **`rot_axes` yaw-only knob**. **Subrun 03** now runs the connectome FlowRNN ×20 seeds at the full 300-epoch budget vs a GRU ceiling on the identical yaw-only stimulus — a *learnability* probe, framed so a null means "not plug-and-play for optic flow," **not** "cannot learn." **Subrun 04** (staged) is the substrate-swap control: the *same* yaw task on the **mushroom body** (unsigned 14k full + ~6k core+ALPN, ×20 seeds each = 40 runs, shared GRU ceiling) — if both the optic lobe and the mushroom body floor it's a training-difficulty story, not a vision one; if the optic lobe learns and the mushroom body doesn't, the optic lobe's visual wiring is doing the work. *(Side-finding retained: the connectome keeps activity stable where random rewiring explodes ~1000–2000× → split off as a conditioning follow-up. Translation speed is physically unreadable from one eye, so translation targets are the readable cues, ground-flow + heading.)* | [experiment_vis_01_optic_flow.md](experiment_vis_01_optic_flow.md) |
| mb-06 | 2026-07-08 | MB evidence integration — generic-I/O connectome vs degree-matched controls on a temporal-integration task | **Concluded 2026-07-09. The Exp-1/2 connectome advantage generalizes beyond MQAR.** On the temporal-integration task the connectome beats degree-matched controls on **both** substrates with the connectome's worst seed above every control graph (complete separation): core **0.827** vs 0.725 (+4.31 control-SD), full **0.838** vs 0.739 (+5.72 control-SD); chance 0.333, Bayes ceiling 0.895; perm-p at the 1/21 floor but the load-bearing fact is the complete separation, not the (resolution-limited) p. Wins on both secondary per-category recalls too (neutral + polar). Matching held per-run (ρ=0.9500 all 80; identical edges/params/inits/noise streams; RMS residual ≤0.0007). **Scope (both independent reviews converged):** this is *generality of the advantage across task classes*, **not** "better at integration" (the same regime already wins on non-integration MQAR; no within-experiment non-integration control isolates an integration channel — the verifier ablations validate the *task* needs integration, not that the *gap* is caused by it); and **n=1 biological graph** ("this connectome," not "topology as a class"). Correction: the pre-flight "0.716 plateau" was under-trained — real runs reach 0.83–0.84 (~0.06 under ceiling), which only compresses the gap, so it works for the result. Two fresh adversarial reviews found no flaw; task validity re-confirmed (single-shot Bayes 0.589 « observed 0.83; shuffled-evidence → 0.333; K-curve monotone). Follow-up: control-arm ablations + a matched K=1 non-integration control to test integration-specificity; σ/random-Z/3e-3-lr robustness subruns. — Tests whether connectome topology helps when the task REQUIRES temporal integration (vs Exp-5 single-shot binding): read each odor's latent 3-way category {attract/neutral/repulse} out of the *running mean* of K noisy scalar evidence samples spread across a random-interleaved O-odor stream (`e = μ(c) + η`), scored at an end-of-stream query (chance 1/3). Keeps the Exp-5-subrun-01 regime (generic all-neuron I/O + degree-matched controls + genuine training-seed replicates) and swaps **only the task** — and is the **clean, off-ceiling, RMS-matched** follow-up subrun-01 itself flagged as needed (subrun-01 won but near-ceiling under ρ-only matching). Backprop only, `core_alpn` + `full`, 20 connectome seeds vs 20 control graphs, lr 1e-3 → **80 runs**; two decoupled noise sources (odor identity easy; evidence σ the primary cap knob); matching = params + degree/weight multiset + ρ=0.95 **+ activation-RMS match via a non-recurrent input-gain lever** (an independent review caught that the first build's RMS match scaled the operator and dragged control ρ 0.95→~0.76 on real data; the fix holds ρ=0.95 for both arms, verified); permutation-rank primary led by control-SD effect sizes; verifier ablations (first-only / shuffled-evidence / K-curve / Bayes bound) prove the task needs integration. **Pre-flight complete + passing (2026-07-08):** both substrates plateau in-band at **0.716** (off the 0.895 Bayes ceiling; σ=1.0 unchanged); verifier confirms integration on both (first-only 0.44/0.41, shuffle→chance 0.33, K-curve 0.39→0.71); lr sweep kept 1e-3 (3e-4 0.66 / 1e-3 0.72 / 3e-3 0.76 but closer to ceiling — 3e-3 replication noted as a future robustness subrun). **Pre-registered:** the integration task class was chosen as most favourable to topology, so a connectome win must survive robustness checks; a tie is the strong result. Awaiting `run.py --collect`. | [experiment_06_mb_evidence_integration.md](experiment_06_mb_evidence_integration.md) |
...
matched extends the Exp-4/5 "no wiring advantage" null to a third task family. **Answer (concluded
2026-07-09):** the connectome wins decisively on both substrates (core 0.827 vs 0.725, full 0.838 vs
0.739; complete separation, 4.3–5.7 control-SD) — **the Exp-1/2 advantage generalizes to a new task
class.** But scoped honestly (both independent reviews agreed): this shows the advantage *generalizes
across tasks*, not that the connectome is *better at integration* (the same regime already wins on
non-integration MQAR, and there is no within-experiment non-integration control), and it remains n=1
biological graph. The pre-flight band was an under-trained read (real runs 0.83–0.84, near but under
the 0.895 ceiling — which only shrinks the measured gap). See the entry for the full write-up + figures.
Code: [`scott/experiment_06_mb_evidence_integration/`](../experiment_06_mb_evidence_integration/).

### vis-01 — one-line description
Every result so far (mb-01…06) is in the **mushroom body**. Does the connectome advantage carry to a
**different brain region** and a **physical, real-world task**? vis-01 is the vision analogue of mb-01 and
the go/no-go gate for the optic-lobe (`vis`) track: does the FlyWire **left optic-lobe** connectome's
wiring beat random rewiring at reading the fly's own motion from a fly-eye movie? The network watches ~44
frames from a fly-like hexagonal eye flying through a 3D world — a far background, a ground plane, and
**dense near-field clutter (48 objects at fixed random depths, 0.3–3 m)** to supply motion cues — and
reports, each frame, how the fly is moving. **Current design:** the fly **continuously turns** (yaw/roll/
pitch — the classic "optomotor" computation) and translates, in **separate turn-only and translate-only
trials** so turning (which makes big image motion) doesn't swamp translation (small image motion). Two
facts settled the readout: absolute forward/sideways *speed* is physically unreadable from one eye (image
motion = speed ÷ distance, so the same picture can mean fast-far or slow-near), so translation targets are
the *readable* cues the fly actually uses (ground-flow rate = speed÷height, and heading); and even those
are weak at cruising speed, so translation gets its own trials and the **current headline claim is turning
(rotation) estimation**. The network's recurrent connections are the real connectome (~4.2M trainable
connections); the control is the same wiring randomly reshuffled (same per-neuron connection counts). Both
get **activity normalization** — a gain-control step that holds each network's activity at a steady level —
so the comparison is about the wiring *shape*, not about whether one network's activity blows up. That last
point is itself a result: at equal gain the connectome keeps activity stable where random rewiring explodes
~1000–2000× — **split off as a follow-up experiment** on the connectome's conditioning. Engine validated on
the real 48,894-neuron substrate (memory-efficient gradient → 0.5 GB; balanced loss; physics checked —
turning drives horizontal image motion, pitch vertical). **Subruns: 02 definitive** (connectome ×20 vs
control ×20, 5-DOF) **ran on the AWS fleet and both arms floored at val R² ≈ 0** — a training/model failure,
not a wiring signal. Debugging on a reduced **yaw-only** stimulus showed the signal *is* learnable (a GRU
reference reaches R² ≈ 0.74) while the sparse network memorizes a batch (0.92) but doesn't generalize, and
the shipped activity-normalization default was diverging (real bug, now fixed by detaching the RMS-norm
denominator; yaw-only is now a first-class `rot_axes` config). **03 yaw-only** is staged: the connectome ×20
seeds at the full 300-epoch budget vs a GRU ceiling on the identical stimulus. **04 mushroom-body swap** is
staged alongside it: the *same* yaw task run on the mushroom body instead of the optic lobe (unsigned 14k
full + ~6k core+ALPN, ×20 seeds each; the GRU ceiling is shared since the task is unchanged) — a control
that separates "vision-specific" from "hard-to-train-in-general" if the optic lobe floors. **Answer (subruns
03 + 04, 2026-07-12): both floored — the difficulty is hard-to-train-in-general, not vision.** All 60
connectome networks (optic lobe + both mushroom-body arms) sat at held-out R² ≈ 0 through the full 300-epoch
budget while a GRU read the same yaw stimulus at 0.58 (causal) / 0.76 (bidirectional). Because the non-visual
mushroom body floors exactly as the optic lobe does, the blocker is training these sparse connectome FlowRNNs
on continuous regression (the recurrent state collapses to a fixed point and the readout emits the
per-episode mean), not the optic lobe's wiring. The learnability-probe framing holds: this says the
connectome is *not plug-and-play* for optic flow, not that it cannot learn. The headline
connectome-vs-degree-matched comparison (subrun 02) is therefore **blocked** until a model/training fix lifts
a substrate above floor — to be developed and validated on the cheap `mb_core_alpn` (~3 h/run) before any
optic-lobe rerun (~26 h/run).
Code: [`scott/experiment_vis_01_optic_flow/`](../experiment_vis_01_optic_flow/).
