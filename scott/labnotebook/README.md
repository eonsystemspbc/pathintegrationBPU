...
| 2 | 2026-06-19 | MB-core pruning vs the full 14k FlyWire substrate on MQAR | **Concluded 2026-06-21.** Pruning the 14k substrate to the ~5.6k canonical MB core keeps the advantage: core **0.881 ± 0.012** beats every control — degree-matched core 0.701, random 5.6k subset 0.838, and the **14k degree-matched control 0.769** (each permutation p = 0.048) — and trains **~2.5× faster in wall-clock** than the full 14k (0.919) for ~0.04 less accuracy. Exp 1's finding holds at core scale (the wiring effect is intrinsic to the core, not the halo); `full` vs `full_degree` reproduces Exp 1. Next → Exp 3: biological PN/KC→MBON I/O. A cell-type join (Schlegel 2024, FlyWire 783) shows the Exp-1 "mushroom_body" substrate (14,025 neurons) is an MB-neuropil-anchored subgraph: a ~5.6k MB core (KC/MBON/DAN/MBIN) plus an ~8.4k weakly-attached halo (639 central-complex neurons + ~7.1k unlabeled). Prunes to the MB core and asks, at matched ρ=0.95: (1) does Exp 1 hold? core vs degree-matched cores; (2) right subset, not just smaller? core vs random 5.6k subsets; (3) what does pruning buy? core vs full — accuracy + grok + wall-clock. 400 runs (20 core + 20 full + 20×2 controls × 5 lr) on the AWS fleet. Engine/launcher validated locally; not yet launched. Generic all-neuron I/O (bio I/O → Exp 3). | [experiment_02_mb_core_pruning.md](experiment_02_mb_core_pruning.md) |
