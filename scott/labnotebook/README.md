...
| 3 | 2026-06-24 | Dense parameter-matched controls vs the connectome on MQAR | **Launched on the AWS fleet 2026-06-24 (running); results pending.** Reframes the connectome-advantage question as parameter budget: against dense controls at a matched *trainable-parameter* budget, does the connectome's specific sparse wiring still pay off? Connectome arms (`core` 0.881, `full` 0.919) are pulled in from Exp 2 (lr=1e-3, not retrained); three dense controls are trained per substrate at lr=1e-3, gain-matched by activation-RMS: **C1** dense same-N 100%-trainable (size-matched ceiling, far more params), **C2** dense frozen random scaffold + E=nnz(connectome) trainable deltas (trainable-param-matched — the random-directions dense reservoir / matched topology test, permutation null), **C3** smaller dense net at matched *total* trainable params (budget in fewer neurons). Supplies the random-directions dense-reservoir baseline Exp 2's eigvec arm lacked (which kept the connectome's directions and beat the full connectome 0.964 vs 0.919). 120 control runs (20×3×2 substrates), 64-GPU fleet, lr=1e-3, patience off. Results pending. | [experiment_03_dense_param_matched.md](experiment_03_dense_param_matched.md) |
...
**Answer: pending** — launched on the AWS fleet 2026-06-24, results to come.
