#!/usr/bin/env python3
"""run.py - one-command launcher for the FULL Experiment 4 run on the AWS spot-GPU fleet.

Experiment 4: BIOLOGICAL MB I/O on MQAR, across FOUR learning paradigms on the identical
substrate + biological ports (SPEC.md). Input->ALPN, teaching->DAN, readout<-MBON;
recurrence biologically-forward (operator = M, post x pre storage); every condition rho-matched to 0.95.
...
Controls share the EXACT ALPN/KC/MBON/DAN/MBIN port index sets; only the wiring differs
(degree-preserving rewiring, then rho=0.95). The connectome is one graph x SEEDS training-seed
replicates (pseudo-replication) -> permutation-rank primary; degree_matched = independent graphs.
...
# ----------------------------------------------------------------------------- run knobs
EPOCHS = 300
PATIENCE = EPOCHS       # plateau early-stop OFF (= epoch cap): keep converged-stop (val>=0.995)
                        # so fast-grokkers stop early and wall-clock stays fair (Exp 2-3 policy).
MICROSTEPS = 2          # recurrence steps/token; PINNED (ALPN->KC needs 2 hops; =1 kills Arm B's code)
ELIG_LAMBDA = 0.3       # PINNED eligibility decay for the HYBRID rule (pure rules sweep LAM_GRID)
ETA = 0.3               # fixed plastic rate (delta + hybrid inner; hebbian is eta-invariant)
SUBSTRATE = "core_alpn"  # PRIMARY substrate (MB core + ALPN input layer). "full" = robustness arm.
# --- sizes (confirm after smoke + reviewer sign-off) ---------------------------------------
...
RULES = ("hebbian", "delta", "hybrid")               #   -- matched tuning; lambda dominates (review)
# ------------------------------------------------------------------------------------------
FLEET_SIZE = 64         # instances = shards; ~16 spot + rest on-demand (Exp 1-3). Tunable knob.
MATRIX = "connectomes/flywire_mushroom_body/adjacency_unsigned.npz"  # the full 14k substrate
S3_PREFIX = "pathint-exp04-bioio"                    # isolated S3 area for this run's outputs
# ------------------------------------------------------------------------------ plumbing
...
EXP_RUN_SCRIPT = "scott/experiment_04_mb_biological_io/run_experiment.py"
EXP_OUTPUT_DIR = "scott/experiment_04_mb_biological_io/outputs"
...
def n_runs() -> int:
    ...
    plast = 0
    for rule in RULES:
        grid = len(LR_GRID) if rule == "hybrid" else len(LAM_GRID)   # hybrid sweeps lr; pure sweep lambda
        plast += (SEEDS + CONTROL_GRAPHS) * grid        # connectome (SEEDS) + degree (GRAPHS)
    return bptt + plast
...
def write_config() -> None:
    ...
    overrides = {
        ...
        "SUBSTRATE_FILES": MATRIX,            # only the 14k adjacency is git-ignored data;
                                              ...
                                              # (git ls-files -co: committed or untracked-not-ignored).
    }
    seen: set[str] = set()
    out_lines = [
        "# GENERATED by run.py - do not hand-edit; edit the constants in run.py instead.",
        "# Overrides aws_fleet/config.env for the full Experiment 4 run.",
        "",
    ]
...
def plan_banner() -> str:
    ...
    return (
        ...
        f"                      pure hebbian/delta sweep lambda {', '.join(LAM_GRID)} (eta={ETA}); "
        f"hybrid sweeps outer lr (lambda={ELIG_LAMBDA})\n"
...
def launch(skip_confirm: bool) -> int:
    ...
    if (rc := sh("launch_fleet.sh")) != 0:
        return rc
    rel = "scott/experiment_04_mb_biological_io/run.py"
...
def status() -> int:
    ...
    print(f"\n=== Experiment 4 progress ({n_runs()} runs planned) ===")
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Full Experiment 4 fleet launcher.")
