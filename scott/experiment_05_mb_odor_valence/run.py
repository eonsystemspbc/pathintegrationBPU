...
LR_GRID = ("1e-4", "3e-4", "1e-3", "3e-3", "1e-2")   # backprop + hybrid-outer lr: Exp-4-parity 5pt
                        # sweep re-enabled: Exp-4 optima were backprop 3e-4 vs hybrid 1e-2 (an order of
                        # magnitude apart), so a single shared point can't serve both; and odor->valence
                        # is a new task, so per the Exp-3 lesson the optima may not transfer. best-by-val.
