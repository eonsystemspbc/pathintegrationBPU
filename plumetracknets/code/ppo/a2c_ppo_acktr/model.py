...
class ConnectomeBPUBase(nn.Module):
    def __init__(
        ...
        if matrix_path is None:
            raise ValueError("--bpu-matrix is required for --policy_type bpu")
        if pools_path is None:
            raise ValueError("--bpu-pools is required for --policy_type bpu")
        ...
        output = pools.loc[pools["pool"] == "output", "index"].astype(int).tolist()
        if not sensory:
            raise ValueError("BPU pool_assignments.csv has no sensory pool")
        if not output:
            raise ValueError("BPU pool_assignments.csv has no output pool")
        if bpu_output_limit and int(bpu_output_limit) > 0:
            output = output[: int(bpu_output_limit)]

        indices = torch.as_tensor(
            np.vstack([recurrent.row, recurrent.col]), dtype=torch.long
        )
        values = torch.as_tensor(recurrent.data, dtype=torch.float32)
        recurrent_tensor = torch.sparse_coo_tensor(
            indices, values, size=recurrent.shape
        ).coalesce()
        self.register_buffer("W_rec_indices", recurrent_tensor.indices())
        self.W_rec_values = nn.Parameter(
            recurrent_tensor.values().clone(), requires_grad=self.train_recurrent
        )
        self.register_buffer("sensory_indices", torch.as_tensor(sensory, dtype=torch.long))
        self.register_buffer("output_indices", torch.as_tensor(output, dtype=torch.long))

        scale_in = 1.0 / np.sqrt(max(num_inputs, 1))
        self.W_in = nn.Parameter(torch.empty(len(sensory), num_inputs))
        self.b_in = nn.Parameter(torch.zeros(len(sensory)))
        nn.init.uniform_(self.W_in, -scale_in, scale_in)

        readout_size = len(output)
        init_ = lambda m: init(m, nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), np.sqrt(2))
        self.actor1 = nn.Sequential(init_(nn.Linear(readout_size, hidden_size)), nn.Tanh())
        self.actor = nn.Sequential(init_(nn.Linear(hidden_size, hidden_size)), nn.Tanh())
        self.critic1 = nn.Sequential(init_(nn.Linear(readout_size, hidden_size)), nn.Tanh())
    ...
    def _bpu_step(self, x, hxs, masks):
        h = hxs * masks
        injection = x @ self.W_in.t() + self.b_in
        W_rec = self._recurrent_matrix()
    ...
    def forward(self, inputs, rnn_hxs, masks):
        if inputs.size(0) == rnn_hxs.size(0):
            return self._bpu_step(inputs, rnn_hxs, masks)
        ...
        for t in range(T):
            value, actor_features, hxs, last_activities = self._bpu_step(
                x[t], hxs, masks[t]
            )
            values.append(value)
