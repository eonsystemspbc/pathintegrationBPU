...
def get_args():
    ...
    parser.add_argument('--bpu-k', dest='bpu_k', type=int, default=None)
    parser.add_argument('--bpu-output-limit', dest='bpu_output_limit', type=int, default=0)
    ...
    args.cuda = (not args.no_cuda) and cuda_available and torch.cuda.device_count() > 0
...
def main():
    ...
    if args.policy_type == 'bpu':
        base_kwargs.update({
            'matrix_path': args.bpu_matrix,
            'pools_path': args.bpu_pools,
            'metadata_path': args.bpu_metadata,
            'bpu_k': args.bpu_k,
